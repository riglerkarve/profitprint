...
## Learned 18 Aug 2026 — the day distribution started

- **Render the built product before it goes anywhere.** The spreadsheet is generated by
  `build_spreadsheet.py` in CI (no Python here): `gh run download` → `scripts/check-delivered-xlsx.cjs`
  (USD numbers) → LibreOffice one-page-per-sheet PDF and *look at it*. Two real defects (an unprefixed
  `_xlfn.MINIFS`, and 37 rows of `$0.00`) were invisible in the code and obvious on the page.
- **The landing page must carry the beacon.** `/tool/` is a separate file from the Astro layout; the
  Cloudflare token has to be in both. It wasn't until 18 Aug, so tool visits before then are
  *unmeasured*, not zero.
- **Reddit replies go via the owner's clipboard, not typed.** `marketing/REPLY-KIT-2026-08.md` +
  `scripts/price-link.cjs` produce the text; Set-Clipboard → Markdown mode → ctrl+v; the owner clicks
  Comment. Typing into Reddit's composer once hid and saved a post through keyboard shortcuts.
- **Coupon codes are the attribution layer** (SEARCH20 / REDDIT20 / GROUPS20): the calculator shows the
  matching one on arrival from utm/referrer; Payhip coupon uses say which channel sold. No pixel, no cookie.
